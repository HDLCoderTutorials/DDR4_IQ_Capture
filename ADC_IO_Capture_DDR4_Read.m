...
DDR4_ReadLen = config.register.ddr4_samples;

% Debug mode:
% If set to true, we capture counter data which is a ramp from 1 to N
% If false we capture ADC data
DebugMode = 0; 
rd_sharedmem = pspshared.libiio.sharedmem.read('IPAddress',IPAddr,'DataType',config.register.ddr4_data_type);
...
function scopeStruct = setupScopes(DebugMode,config,DDR4_ReadLen)
    ...
    hScope = dsp.TimeScope(1, config.synthesis.sample_rate_hz,...
                        'TimeSpanSource', 'Auto', ...
                        'AxesScaling', 'Manual',...
                        'YLimits', YScale,...
                        'LayoutDimensions', [1 1]);
    hSpecAn = dsp.SpectrumAnalyzer( ...
                        'SampleRate', config.synthesis.sample_rate_hz);
    ...
    hSpecAn.WindowLength = DDR4_ReadLen/config.radar.pulses_per_cpi*4; 
...
columnNames = {'Name','Address','DataType','InitialValue'};
    ...
    'CPILength',hex2dec('120'),uint32(0),config.register.pulses_per_cpi;...
    'PulseWidth',hex2dec('124'),uint32(0),config.register.pulse_width_cycles;...
    'PRI',hex2dec('128'),uint32(0),config.register.pri_cycles;...
    'RngGateDelay',hex2dec('12C'),uint32(0),config.register.tx_end_to_rx_start_delay_cycles;...
    'NCO_incr',hex2dec('118'),int32(0),config.register.start_inc_steps;...
    'NCO_DAC_I_Gain',hex2dec('130'),fi(0,numerictype('ufix8_En7')),1;...
    'NCO_DAC_Q_Gain',hex2dec('134'),fi(0,numerictype('ufix8_En7')),1;...
    'NCO_end_incr',hex2dec('138'),int32(0),config.register.end_inc_steps;...
    'NCO_step_value',hex2dec('140'),int32(0),config.register.lfm_counter_inc;...
    'RngSwathLength',hex2dec('13C'),uint32(0),config.register.range_swath_cycles;...
    };
